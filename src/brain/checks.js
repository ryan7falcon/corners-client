...
const isTargetEqual = (t1, t2) => {
  arrayEquals(t1.position, t2.position)
}
...
const allowedToSelectAnother = (state, target) => {
  return (isOwnTarget(state, target) && hasntJumpedYet(state))
}

// Check if player is allowed to deselect the piece
const allowedToDeselect = (state, target) => {
  return (isTargetEqual(state.selectedCell, target) && hasntJumpedYet(state))
}
...
const isValidWalk = (state, target) => {
  return isTargetValid(state.validTargets.walks, target)
}

// check if target is a valid jump for the player
const isValidJump = (state, target) => {
  return isTargetValid(state.validTargets.jumps, target)
}

export { isTargetEmpty, isOwnTarget, hasntJumpedYet, allowedToSelectAnother, allowedToDeselect, isValidWalk, isValidJump }